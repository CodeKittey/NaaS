...
router.get('/api/int', function () {
	var ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee(ctx, next) {
		var query, allowNegative, max, min, count, arrInt, i, data;
		return regeneratorRuntime.wrap(function _callee$(_context) {
			while (1) {
				switch (_context.prev = _context.next) {
					case 0:
						query = ctx.query;
						allowNegative = query['allowNegative'] === 'true';
						max = parseInt(query['max'], 10) || 1000;
						min = (parseInt(query['min'], 10) || 0) < 0 && allowNegative !== true ? 0 : parseInt(query['min'], 10) || 0;
						count = parseInt(query['count'], 10) < 122 ? parseInt(query['count'], 10) : 1;
						arrInt = [];
						arrInt.length = count;
						i = 0;

						while (i < count) {
							arrInt[i] = parseInt(Math.floor(Math.random() * (max - min)) + min, 10);i++;
						}

						data = count > 1 ? arrInt : arrInt[0];
		...
		return regeneratorRuntime.wrap(function _callee2$(_context2) {
			while (1) {
				switch (_context2.prev = _context2.next) {
					case 0:
						data = parseInt(ctx.params.id, 10);
...
router.get('/api/float', function () {
	var ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee3(ctx, next) {
		var query, precision, allowNegative, min, max, count, arrFloat, i, data;
		return regeneratorRuntime.wrap(function _callee3$(_context3) {
			while (1) {
				switch (_context3.prev = _context3.next) {
					case 0:
						query = ctx.query;
						precision = parseInt(query['precision'], 10) < 21 ? parseInt(query['precision']) : 2;
						allowNegative = query['allowNegative'] === 'true';
						min = (parseInt(query['min'], 10) || 0) < 0 && allowNegative !== true ? 0 : parseInt(query['min'], 10) || 0;
						max = parseInt(query['max'], 10) || 1000;
						count = parseInt(query['count'], 10) < 122 ? parseInt(query['count'], 10) : 1;
						arrFloat = [];
						arrFloat.length = count;
						i = 0;

						while (i < count) {
							arrFloat[i] = parseFloat((Math.random() * (max - min) + min).toFixed(precision));i++;
						}

						data = count > 1 ? arrFloat : arrFloat[0];
		...
		return regeneratorRuntime.wrap(function _callee4$(_context4) {
			while (1) {
				switch (_context4.prev = _context4.next) {
					case 0:
						query = ctx.query;
						precision = parseInt(query['precision'], 10) || 2;
						id = parseFloat(ctx.params.id);
						//TODO get the saved float

						data = parseFloat(id.toFixed(precision));
