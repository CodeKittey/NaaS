...
router.get('/api/double', function () {
	var ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee3(ctx, next) {
		var query, dez, min, max, frac, data;
		return regeneratorRuntime.wrap(function _callee3$(_context3) {
			while (1) {
				switch (_context3.prev = _context3.next) {
					case 0:
						query = ctx.query;
						dez = parseInt(query['dez']) || 2;
						min = parseInt(query['min']) || 0;
						max = parseInt(query['max']) || 1000;
						frac = Math.pow(10, dez);
						data = Math.round((Math.random() * (max - min) + min) * frac) / frac;
...
router.get('/api/double/:id', function () {
				...
				switch (_context4.prev = _context4.next) {
					case 0:
						...
						if (data !== null) {
							...
							ctx.body = { type: 'double', value: data, frac: frac };
							ctx.status = 200;
						} else {
